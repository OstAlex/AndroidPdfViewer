...
public class PDFView extends RelativeLayout {
    ...
    public void jumpTo(int page, boolean withAnimation, boolean isCenter) {
        ...
        Log.d(TAG, "jumpTo() called with: page = [" + page + "], withAnimation = [" + withAnimation + "], isCenter = [" + isCenter + "]");
    ...
    public void toFitPage() {
        if (!pageFling && !swipeVertical) {
            int page = getCurrentPage();
            float currentXOffset = getCurrentXOffset();
            float pageOffset = pdfFile.getPageOffset(page, getZoom());
            SizeF pageSize = pdfFile.getPageSize(page);
            if (currentXOffset > pageOffset + pageSize.getWidth() / 2 && page + 1 < pdfFile.getPagesCount()) {
                jumpTo(page + 1, true, true);
            } else {
                jumpTo(page, true, true);
            }
        }
    }
